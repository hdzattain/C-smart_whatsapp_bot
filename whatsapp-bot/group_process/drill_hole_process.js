...
const { DRILL_GROUPS } = require('../group_constants.js');

// 检查是否是目标群组（外墙棚架群组）
function isTargetGroup(groupId) {
  return DRILL_GROUPS.includes(groupId);
}
...
async function processDrillingQuery(query, groupId, contactPhone) {
  // 保存原始query用于CSV记录
...
const drill_conditions = [
  {
    ...
    action: (query, groupId) => handleApply(query, groupId, contactPhone),
  },
  {
    test: query => /(圍封|看守|防墮|眼罩|耳塞|安全帶|围封|防坠|安全带)/.test(query),
    action: (query, groupId) => handleSafety(query, groupId, contactPhone),
  },
  {
...
async function handleApply(query, groupId) {// 修正后的代码
  ...
  try {
    ...
    if (isSuccess) {
      ...
      replyStr = `申請失敗，請重試`;
      console.log(`群组id: ${groupId}, 打窿群組-申请流程失败： ${errorMsg}`);
      appendLog(groupId, `打窿群組-申请流程失败： ${errorMsg}`);
      await logOperationToCsvIfTargetGroup({
        query: originalQuery,
        category: '申请',
        groupId,
...
async function handleSafety(query, groupId, contactPhone) {
  ...
  // 正则匹配用户输入，提取字段值
  const matches = extractFields(query, fields);
  console.log(`群组id: ${groupId}, 发送人手机号: ${contactPhone}, 安全相更新类型: ${senderType}, 打窿群組 - 安全相更新匹配的字段值： ${JSON.stringify(matches)}`);
  appendLog(groupId, `发送人手机号: ${contactPhone}, 安全相更新类型: ${senderType}, 打窿群組 - 安全相更新匹配的字段值： ${JSON.stringify(matches)}`);
